...
> **Version compatibility**: designed and tested with CC Switch **3.20.3** (database schema v18, no migration since 3.20.1); CC Switch 3.20.1–3.20.2 (schema v18) and 3.20.0 (schema v17) remain compatible. Run `scripts/ccs_db.py doctor` to verify the installed schema; other versions may behave differently — see `references/migration.md` for official version behavior changes.
...
## 8. Proactive preflight and known upstream defect (3.20.0–3.20.3)
...
- **Known upstream defect**: CC Switch 3.20.0's provider edit page — even when saved without any changes — can reorder `config.toml`, strip common-config blocks, misplace markers, and serialize a url-only remote MCP as `type="stdio"` + `command=""` (frontend smol-toml round-trip plus backend toml_edit strip/merge). **Still not fixed as of 3.20.3**: upstream #6719 is open, and neither the 3.20.3 release notes nor its commits contain an edit-page fix — the warning covers 3.20.0–3.20.3. **Do not use the edit page to save or extract Codex provider config**; use this skill's commands instead.
