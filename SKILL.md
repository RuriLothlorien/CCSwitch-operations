...
- **Scope disambiguation**: "sync to CC Switch" / "同步 CCS" is ambiguous. Only touch the `common-config` snippet when the user explicitly says "common config" / "通用配置". If the scope is not explicit (provider config vs common config vs both), **ask first** — never unilaterally decide to change only the common-config.
