...
## 8. Proactive preflight and known upstream defect (3.20.0–3.20.2)
...
- **Known upstream defect**: CC Switch 3.20.0's provider edit page — even when saved without any changes — can reorder `config.toml`, strip common-config blocks, misplace markers, and serialize a url-only remote MCP as `type="stdio"` + `command=""` (frontend smol-toml round-trip plus backend toml_edit strip/merge). **This is still not in the 3.20.2 fix list** (upstream #6719). **Do not use the edit page to save or extract Codex provider config**; use this skill's commands instead.
